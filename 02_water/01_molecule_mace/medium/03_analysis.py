...
def __(filenames_sorted, pd):
    ...
    df
    return df, df_tmp, filename


@app.cell
def __(df):
    df[df["#"] == 0]["cm^-1"].values[0]
    return


@app.cell
def __(df):
    df[df["#"] == 1]["cm^-1"].values[0]
    return
...
def __(df):
    ...
    for name, group in groups:
        plt.plot(group["delta"], group["freq"], marker="x", linestyle="-", label=name)
    # Logscale on x
    plt.xscale("log")
    ...
    # Title
    plt.title("Frequenze vs spostamento")
    # Labels
