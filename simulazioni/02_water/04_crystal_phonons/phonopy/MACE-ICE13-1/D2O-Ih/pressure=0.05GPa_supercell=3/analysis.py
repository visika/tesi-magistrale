...
def __(mo):
    mo.md(
        """
        # Studio dei modi vibrazionali del polimorfo Ih del ghiaccio pesante

        Si sono calcolate le forze e la matrice delle costanti di forza del cristallo Ih del \(\mathrm{D_2O}\) col metodo degli spostamenti finiti,
        usando una supercella \(3 \\times 3 \\times 3\).
...
def __(connections, labels, phonopy, qpoints):
    mace_ice13_1_h2o = {}
    mace_ice13_1_h2o["basepath"] = (
        "/home/mariano/Progetti/tesi-magistrale/"
        "simulazioni/02_water/04_crystal_phonons/phonopy/MACE-ICE13-1/ICE-Ih/supercell=3/"
    )
    mace_ice13_1_h2o["ph"] = phonopy.load(
        mace_ice13_1_h2o["basepath"] + "phonopy_params.yaml"
    )
    mace_ice13_1_h2o["ph"].run_band_structure(
        qpoints, path_connections=connections, labels=labels, is_legacy_plot=False
    )
    return mace_ice13_1_h2o,
...
@app.cell
def __(connections, labels, mace_ice13_1_h2o, mace_ice13_1_s3, plt):
    ...
    n = len(
        [x for x in mace_ice13_1_s3["ph"]._band_structure.path_connections if not x]
    )
    ...
    # _plot(mace_ice13_1_s3["ph"], fmt="r-")

    _dict = mace_ice13_1_h2o["ph"].get_band_structure_dict()
    _frequencies = _dict["frequencies"]
    _distances = _dict["distances"]
    ...
    _fig.suptitle("Supercell $3 \\times 3 \\times 3$")
...
@app.cell
def __(Line2D, band_plot, connections, labels, mace_ice13_1_s3, plt):
    _n = len(
        [x for x in mace_ice13_1_s3["ph"]._band_structure.path_connections if not x]
    )
...
@app.cell
def __(mo):
    mesh = mo.ui.slider(1, 50, value=4, show_value=True).form()
    return mesh,
...
@app.cell
def __(mace_ice13_1_s3, mesh, plt):
    _sigma = 0.05
    ...
    ax.plot(
        ...
        total_dos["total_dos"],
        color="green",
        label="MACE-ICE13-1 D2O 0.05 GPa Supercell 3x3x3",
        zorder=20,
    )

    # mace_ice13_1_h2o["ph"].run_mesh([mesh.value, mesh.value, mesh.value])
    # mace_ice13_1_h2o["ph"].run_total_dos(
    #     sigma=_sigma,
    #     freq_min=0,
    #     freq_max=14,
    #     freq_pitch=None,
    #     use_tetrahedron_method=True,
    # )
    # total_dos = mace_ice13_1_h2o["ph"].get_total_dos_dict()

    # ax.plot(
    #     total_dos["frequency_points"],
    #     total_dos["total_dos"],
    #     color="blue",
    #     label="MACE-ICE13-1 H2O",
    #     zorder=20,
    # )
