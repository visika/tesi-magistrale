...
# calculator =choose_calculator(architecture=arch,use_device=device)
...
if dos:
    ...
    result["phonon"].write_total_dos(filename=f"{sysname}-dos-{model}.dat")
    dt = clock.datetime.now() - t
    print(f"phonons dos finished after {dt}")

    t = clock.datetime.now()
    print(f"phonons tight dos started at {t}")
    bplt_tight = result["phonon"].plot_total_dos(with_tight_frequency_range=True)
    bplt_tight.savefig(f"{sysname}-dos-tight-{model}.pdf")
    dt = clock.datetime.now() - t
    print(f"phonons tight dos finished after {dt}")
    ...
    result["phonon"].write_projected_dos(filename=f"{sysname}-pdos-{model}.dat")
    bplt_tight = result["phonon"].plot_projected_dos(with_tight_frequency_range=True)
    bplt_tight.savefig(f"{sysname}-pdos-tight-{model}.pdf")
    dt = clock.datetime.now() - t
    print(f"phonons pdos finished after {dt}")

global_dt = clock.datetime.now() - global_t
print(f"Python calculations since optimization start finished after {global_dt}")
