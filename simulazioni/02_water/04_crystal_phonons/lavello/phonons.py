...
phonon.forces = [ calc_forces(calculator, supercell) for supercell in disp_supercells if supercell is not None ]
dt = clock.datetime.now() - t
...
result['phonon'].auto_band_structure(write_yaml=True,filename=f"{sysname}-auto-band-{model}.yaml")
